...
module.exports = function sailsHookRabbitMQ(sails) {
  return {
    ...
    initialize: async function initialize(next) {
      ...
      let routes = [];
      if (sails.config.rabbitmq.routes instanceof Function) {
        routes = sails.config.rabbitmq.routes();
      } else {
        routes = sails.config.rabbitmq.routes;
      }
      ...
      global[sails.config.rabbitmq.customModelGlobal].publish = async function(exchange, routingKey, content, options = undefined) {
        ...
        if (!!sails.config.rabbitmq.waitForServerToAcceptMessages) return that.channels.default.publish(exchange, routingKey, content, options);
        return new Promise(resolve => {
          that.channels.default.publish(exchange, routingKey, content, options).catch(err => {
      ...
      global[sails.config.rabbitmq.customModelGlobal].sendToQueue = async function(queue, content, options = undefined) {
        ...
        if (!!sails.config.rabbitmq.waitForServerToAcceptMessages) return that.channels.default.sendToQueue(queue, content, options);
        return new Promise((resolve, reject) => {
          let resp = that.channels.default.sendToQueue(queue, content, options).catch(err => {
    ...
    /**
     * Connect to and initialize the AMQP server.
     * @returns {Promise<void>}
     */
    initializeAmqp: function () {
      sails.log.info('Rabbit MQ - Initializing `amqp`...');
      let that = this;
      return new Promise(resolve => {
        ...
        global[sails.config.rabbitmq.customModelGlobal].connection = this.connection;

        // Monitor connection events
        this.connection.on('connect', function (t) {
          sails.log.debug('Rabbit MQ - Attempting connection');
          that.connected = true;
    ...
    /**
     * Validate the connection to MQ server. If a successful
     * connection message is not received within 5 seconds
     * throw a failed connection error
     */
